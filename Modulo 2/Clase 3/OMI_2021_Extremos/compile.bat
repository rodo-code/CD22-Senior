...
SET TARGET=libinteractive\extremos\extremos.exe
SET SOURCES=extremos.cpp libinteractive\extremos\extremos_entry.cpp
call:recompile %TARGET% %SOURCES%
IF "%RECOMPILE%" == "1" (
<NUL SET /p elidenewline=Compiling %TARGET%... 
%GXX% -o %TARGET% %SOURCES% -std=c++11 -lm -O2 -g -Wno-unused-result -Ilibinteractive/extremos
IF !ERRORLEVEL! NEQ 0 EXIT /B !ERRORLEVEL!
ECHO DONE
)

SET TARGET=libinteractive\extremos\extremos_debug.exe
SET SOURCES=extremos.cpp libinteractive\extremos\extremos_entry.cpp
call:recompile %TARGET% %SOURCES%
IF "%RECOMPILE%" == "1" (
<NUL SET /p elidenewline=Compiling %TARGET%... 
%GXX% -o %TARGET% %SOURCES% -std=c++11 -lm -g -Wno-unused-result -Ilibinteractive/extremos
IF !ERRORLEVEL! NEQ 0 EXIT /B !ERRORLEVEL!
ECHO DONE
)
