...
static struct __stream __extremos_in, __extremos_out;

void __NOINLINE __libinteractive_init() {
	openstream(&__extremos_in, "\\\\.\\pipe\\libinteractive_Main_extremos_in", O_WRONLY);
	openstream(&__extremos_out, "\\\\.\\pipe\\libinteractive_Main_extremos_out", O_RDONLY);
}
...
static void __message_loop_extremos(int __current_function, int __noreturn) {
	uint32_t __msgid;
	while (readfull(&__extremos_out, &__msgid, sizeof(__msgid), 0)) {
		if (__msgid == __current_function) return;
		switch (__msgid) {
			case 0x71651df7u: {
				// extremos -> Main.esMenor
				uint32_t __cookie;
				int esMenor_i;
				readfull(&__extremos_out, &esMenor_i, sizeof(int), 1);
				int esMenor_j;
				readfull(&__extremos_out, &esMenor_j, sizeof(int), 1);
				readfull(&__extremos_out, &__cookie, sizeof(__cookie), 1);

				bool result =
				esMenor(esMenor_i, esMenor_j);

				writefull(&__extremos_in, &__msgid, sizeof(__msgid));
				writefull(&__extremos_in, &result, sizeof(result));
				writefull(&__extremos_in, &__cookie, sizeof(__cookie));
				writeflush(&__extremos_in);
				break;
			}

			case 0x685d93e7u: {
				// extremos -> Main.respuesta
				uint32_t __cookie;
				int respuesta_posMenor;
				readfull(&__extremos_out, &respuesta_posMenor, sizeof(int), 1);
				int respuesta_posMayor;
				readfull(&__extremos_out, &respuesta_posMayor, sizeof(int), 1);
				readfull(&__extremos_out, &__cookie, sizeof(__cookie), 1);

				respuesta(respuesta_posMenor, respuesta_posMayor);
...
void buscaExtremos(int n) {
	...
	writefull(&__extremos_in, &__msgid, sizeof(__msgid));
	writefull(&__extremos_in, &n, sizeof(int));
	writefull(&__extremos_in, &__cookie, sizeof(__cookie));
	writeflush(&__extremos_in);

	__message_loop_extremos(__msgid,  0 );
	uint32_t __cookie_result = 0;
	readfull(&__extremos_out, &__cookie_result, sizeof(__cookie_result), 1);
