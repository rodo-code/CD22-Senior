...
static void __message_loop_Main(int __current_function, int __noreturn) {
	...
	while (readfull(&__in, &__msgid, sizeof(__msgid), 0)) {
		...
		switch (__msgid) {
			case 0x13857295u: {
				// Main -> extremos.buscaExtremos
				uint32_t __cookie;
				int buscaExtremos_n;
				readfull(&__in, &buscaExtremos_n, sizeof(int), 1);
				readfull(&__in, &__cookie, sizeof(__cookie), 1);

				buscaExtremos(buscaExtremos_n);
...
void respuesta(int posMenor, int posMayor) {
	...
	writefull(&__out, &__msgid, sizeof(__msgid));
	writefull(&__out, &posMenor, sizeof(int));
	writefull(&__out, &posMayor, sizeof(int));
	writefull(&__out, &__cookie, sizeof(__cookie));
	writeflush(&__out);
...
int main(int argc, char* argv[]) {
	...
	openstream(&__in, "\\\\.\\pipe\\libinteractive_extremos_extremos_in", O_RDONLY);
	openstream(&__out, "\\\\.\\pipe\\libinteractive_extremos_extremos_out", O_WRONLY);
